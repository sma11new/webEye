   ...
   \_____|\___|\__| \/  \/ \___|_.__/_____/|_|\__\___|_|  |_|\__|_|\___|
        ...
        for port in self.args.port:
            ...
            try:
                ...
                if rep.status_code == 200:
                    title = re.findall('<title>(.*)</title>', rep.text)[0]
                    # msg = f"{str(rep.status_code):<5}{reqURL:<30}{title}"
                    msg = f"{reqURL:<30}{title}"
                    self.lock.acquire()
                    try:
                        self.titleList.append(msg)
    ...
    def __del__(self):
        try:
            ...
            self.output()
            print(f"\nThe result has been saved in {self.args.output}\n")
        except:
            pass
